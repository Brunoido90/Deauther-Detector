#!/usr/bin/env python3
# deauth_pro_live.py – Start/Stop-Button + Live-RSSI
...
def set_monitor(iface):
    subprocess.run(["pkill", "-9", "wpa_supplicant"], stderr=subprocess.DEVNULL)
    subprocess.run(["pkill", "-9", "NetworkManager"], stderr=subprocess.DEVNULL)
...
class DeauthApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Deauth Live Detector")
        self.root.geometry("1500x900")
        self.theme = {"bg":"#1e1e1e","fg":"#ffffff","accent":"#0078d4"}
        self.build_ui()
    ...
    def build_ui(self):
        ...
        self.start_btn = ttk.Button(af, text="Start", command=self.start_scan)
        self.start_btn.pack(side=tk.LEFT, padx=5)
        self.stop_btn = ttk.Button(af, text="Stop", command=self.stop_scan, state=tk.DISABLED)
        ...
        bg, fg, acc = self.theme["bg"], self.theme["fg"], self.theme["accent"]
    ...
    def refresh(self):
        ...
        with model.lock:
            ...
            if top:
                macs, counts = zip(*top)
                self.ax.bar(macs, counts, color=self.theme["accent"])
            self.ax.tick_params(colors=self.theme["fg"])
            self.ax.set_title("Live Deauth-Rate pro MAC (letzte 60 Sek.)", color=self.theme["fg"])
    ...
    def start_scan(self):
        ...
        global IFACE_MON
        IFACE_MON = iface
        self.root.title(f"Deauth Live – {iface}")
        STOP_EVENT.clear()
